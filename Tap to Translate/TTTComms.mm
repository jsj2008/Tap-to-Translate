...
+ (void)requestTranslationForText:(NSString*)text completion:(void(^)(NSDictionary*))block
{
	...
	NSString *defaultLanguage = [TTTPrefs getPreferenceForKey:@"defaultLanguage"];
	NSDictionary *request = @{@"sourceLanguage": @"auto",
							  @"sourceText": text,
							  @"targetLanguage": [defaultLanguage isEqualToString:@"auto"] ? [TTTDeviceInfo currentLanguage] : defaultLanguage};
...
+ (void)requestTranslation:(NSDictionary*)request completion:(void(^)(NSDictionary*))block
{
	...
	NSLog(@"tttipc: %@", objc_getClass("TTTIPC"));
...
- (void)requestManager:(id)arg1 didFinishTranslation:(TranslatedItem*)arg2 error:(NSError*)arg3
{
	...
	if(!arg3) {
		NSDictionary *info = @{@"destination": latestRequestApp.copy,
							   @"json": [self dictionaryFromTranslatedItem:arg2]};
