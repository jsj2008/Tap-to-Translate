...
#define TTTIPC_AUTODISCONNECT_TICKTIME 10.0
#define TTTIPC_AUTODISCONNECT_TICKS 6
...
@implementation TTTIPCConnection
...
- (void)sendMessage:(TTTIPCMessage *)message {
	...
	TTTIPCReplyHandler replyHandler = message.replyHandler;
...
- (void)closeConnection {
	...
	if (_replyHandlers != nil && [_replyHandlers count] > 0) {
		for (NSString *key in _replyHandlers) {
			TTTIPCReplyHandler handler = _replyHandlers[key];
			if (handler != nil) {
				executeOnMainThread(^{
					handler(nil);
				});
	...
	[[TTTIPC sharedInstance] notifyConnectionIsClosed:self];
}

- (void)setIncomingMessageHandler:(TTTIPCIncomingMessageHandler)handler forMessageName:(NSString *)messageName {
...
- (TTTIPCIncomingMessageHandler)incomingMessageHandlerForMessageName:(NSString *)messageName {
	TTTIPCIncomingMessageHandler appHandler = _incomingMessageHandlers[messageName];
	if (appHandler == nil) {
		return [TTTIPC sharedInstance].globalIncomingMessageHandlers[messageName];
	} else {
		return appHandler;
	}
}
...
- (void)_handshakeWithSpringBoard {
	...
	TTTIPCMessage *message = [TTTIPCMessage handshakeMessageWithDictionary:dict];
...
- (void)_handshakeWithSpringBoardComplete:(NSDictionary *)dict {
	...
	if (success) {
		...
		[[TTTIPC sharedInstance] notifyConnectionBecomesActive:self];
...
- (NSDictionary *)_handshakeWithApp:(NSDictionary *)dict {
	...
	[TTTIPC setAppWithIdentifier:appIdentifier inBackground:YES];
...
- (void)_readIncomingMessageData {
	...
	if (!_receivedHeader) {
		...
		int headerSize = sizeof(TTTIPCMessageHeader);
		...
		if (_receivedHeaderLength == headerSize) {
			...
			[_receivedHeaderData getBytes:&header length:sizeof(TTTIPCMessageHeader)];
...
- (void)_dispatchReceivedMessage {
	...
	if (isHandshake) {
		...
		if ([TTTIPC isSpringBoard]) {
			NSDictionary *reply = [self _handshakeWithApp:dictionary];
			if (reply != nil) {
				// send a handshake completion message to app
				TTTIPCMessage *message = [TTTIPCMessage handshakeMessageWithDictionary:reply];
				[self sendMessage:message];
			}
		} else {
		...
		TTTIPCReplyHandler handler = _replyHandlers[identifier];
		if (handler != nil) {
...
- (void)_dispatchIncomingMessage:(NSDictionary *)dictionary {
	...
	// handler
	TTTIPCIncomingMessageHandler handler = [self incomingMessageHandlerForMessageName:name];
	...
	if (!_closedConnection) {
		...
		TTTIPCMessage *message = [TTTIPCMessage outgoingMessageWithMessageName:name dictionary:reply messageIdentifier:identifier isReply:YES replyHandler:nil];
		[self sendMessage:message];
	}
}
...
- (void)_createAutoDisconnectTimer {
	if (_autoDisconnectTimer == nil) {
		// ticks every minute
		_autoDisconnectTimer = [[NSTimer scheduledTimerWithTimeInterval:TTTIPC_AUTODISCONNECT_TICKTIME target:self selector:@selector(_autoDisconnectTimerTicks) userInfo:nil repeats:YES] retain];
	}
}
...
- (void)_autoDisconnectTimerTicks {
	@synchronized(self) {
		...
		if (_autoDisconnectTimerTicks == TTTIPC_AUTODISCONNECT_TICKS) {
...
- (void)dealloc {
	...
	// this is for TTTIPC to identify this connection
